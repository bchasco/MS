...
ch <- combined_data[,] %>%
  ...
  mutate(stage = ifelse(event_day<=181,"yr","yr")) %>% #<July 1 is a yearling
  group_by(`Tag Code`,
           init_year,
...
myvars <- c('loc','stage', 'Release Site', "Event Species Name", "ind_len")

# myvars <- c('loc','tagSite', 'stage')
ch_complete <- ch[,] %>%
  group_by(`Tag Code`) %>%
  # mutate(
  #   ind_len = if (all(is.na(`Event Length mm`*1.))) mean(ch$`Event Length mm`, na.rm = TRUE) else zoo::na.approx(`Event Length mm`)
  # )%>%
  mutate(ind_len = if_else(is.na(`Event Length mm`*1.), mean(ch$`Event Length mm`, na.rm = TRUE), `Event Length mm`)) %>%
  group_by(`Tag Code`) %>%
  ...
  select(`Tag Code`, stage, IDnum, loc_code, `Event Species Name`, ind_len) %>%
  ...
  group_by(`Tag Code`, loc, `Event Species Name`, ind_len) %>%
...
# save(WENh, file="data/NASON_allFish_size.rda")
