...
WENh <- WENh %>%
  ...
  mutate(`sp4` = factor("Chinook - wild spring")) %>%
...
WENh <- WENh %>%
  ...
  mutate(`sp2` = ifelse(grepl("Chinook",`Release Site`), ifelse(grepl("HXH",`Release Site`),"Chinook HXH", "Chinook WXW"),
                        ifelse(grepl("Coho",`Release Site`),"Coho",
                               ifelse(grepl("HXH",`Release Site`),"Steelhead HXH", "Steelhead WXW")))) %>%
  mutate(sp2 = factor(sp2, levels = c("Chinook HXH","Chinook WXW", "Coho", "Steelhead HXH", "Steelhead WXW"))) %>%
  mutate(`sp4` = ifelse(grepl("Steelhead",`Release Site`), "Steelhead - hatchery ",
                        ifelse(grepl("Chinook",`Release Site`),
                               ifelse(grepl("LFNH",`Release Site`),"Chinook LNFH - hatchery spring", "Chinook - hatchery spring "),
                               "Coho - hatchery"))) %>%
...
names(WENh)[3] <- "ReleaseSite"
WENh$id <- WENh$id + max(WENtmp$id)

WENh <- rbind(WENtmp,WENh)
WENh$sp4 <- factor(WENh$sp4)

MR_settings <- list(state = 'LH',
                    frms = list(
                                phi = list(yr = ' ~ -1 + loc:ReleaseSite'), #survival
...
g <- ggpubr::ggarrange(phi, p, nrow = 2, align = "v", common.legend = TRUE, legend = "right")
print(g)
