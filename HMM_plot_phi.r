...
x <- x[as.numeric(x$i)%in%groups$id,]
x$ReleaseSite <- fit@TMB$obj$tmb.data$data$ReleaseSite[match(x$i,fit@TMB$obj$tmb.data$data$id)]
x$fSp <- fit@TMB$obj$tmb.data$data$fSp[match(x$i,fit@TMB$obj$tmb.data$data$id)]
