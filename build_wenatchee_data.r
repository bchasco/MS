...
myvars <- c('loc','cum_time','init_year','event_day','tagSite', 'stage')
...
ch_complete <- ch[,] %>%
  ...
  select(`Tag Code`, stage, stageID,IDnum, loc_code, init_year, cum_time, tagSite, event_day) %>%
  ...
  group_by(`Tag Code`, loc, init_year, cum_time, event_day, tagSite) %>%
  ...
  # Fill in year with first capture year for all rows of the fish
  mutate(cum_time = ifelse(is.na(cum_time), NA, cum_time)) %>%
  # Remove duplicates by keeping only the max cum_time for each loc
  group_by(`Tag Code`, loc) %>%
  arrange(desc(cum_time)) %>%  # Arrange cum_time in descending order
...
myCols <- c('loc', 'cum_time', 'init_year', 'event_day', 'tagSite', 'stageID')
